...
- 左側の目次はデスクトップで境界をドラッグして幅を変更でき、ブラウザごとにその幅を保存します。ダブルクリックまたはHomeキーで初期幅へ戻せます。
...
python3 tests/browser_sidebar.py
```
...
`.github/workflows/html-preview.yml`でコード変更時とPages配信後に、実際のブラウザでコピー・検索・内部リンク・数式・スマホ幅・目次幅変更をテストします。
