"""Fast tests for source fidelity, BibTeX extraction and generated internal links."""
...
class SourceTests(unittest.TestCase):
    ...
    def test_generated_site(self):
        with tempfile.TemporaryDirectory() as temp:
            dest = Path(temp)
            m = build.generate(dest, 'test-revision', '', ROOT / 'paper-reading-notes.pdf')
            text = (dest / 'index.html').read_text()
            parsed = Tags(); parsed.feed(text)
            self.assertFalse([k for k, n in Counter(parsed.ids).items() if n > 1])
            self.assertFalse(set(parsed.hrefs) - set(parsed.ids))
            data = json.loads(re.search(r'<script id="bib-data" type="application/json">(.*?)</script>', text, re.S)[1])
            expected = {}
            ...
            actual = {p['path'] for p in m['papers']}
            all_papers = {p.relative_to(ROOT).as_posix() for p in (ROOT / 'papers').glob('*.tex') if not p.name.startswith('_')}
            self.assertEqual(actual, all_papers)
            self.assertEqual(len(parsed.buttons), 3 * (m['paper_count'] + m['reference_count']))
            self.assertEqual((dest / m['pdf_file']).read_bytes(), (ROOT / 'paper-reading-notes.pdf').read_bytes())
